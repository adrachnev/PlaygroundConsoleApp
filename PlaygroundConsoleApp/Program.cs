...
namespace PlaygroundConsoleApp
{
    public class Program
    {
        ...
        private static void ParseModulePlaceholder()
        {
            ...
            string placeHolderText = "ModulePlaceHolder=\"";

            var index = iconMarkup.IndexOf(placeHolderText);
            if (index == -1)
                return;
            if (iconMarkup.Substring(index + placeHolderText.ToCharArray().Length, 1) == "\"") 
            {
                // empty value
                var placeHolderFilled = placeHolderText + Guid.NewGuid();
                iconMarkup =  iconMarkup.Replace(placeHolderText, placeHolderFilled);
